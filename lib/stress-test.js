...
const initClient = async (
  ...
  const audioAction = microphone ? "Microfono" : "Modalità ascoltatore";
  ...
  if (microphone) {
    ...
    await page.waitForSelector('[aria-label="Riattiva"],[aria-label="Silenzia"]');
    // If we are muted, click on Unmute
    const unmuteButton = await page.$('[aria-label="Riattiva"]');
  ...
  if (webcam) {
    await page.waitForSelector('[aria-label="Mostra webcam"]');
    await page.click('[aria-label="Mostra webcam"]');
    logger.debug("clicked on sharing webcam");
    await new Promise((resolve) => setTimeout(resolve, 2000));
    await page.waitForSelector("#setCam > option");
    await page.waitForSelector('[aria-label="Avvia condivisione"]');
    logger.debug("clicking on start sharing");
    await page.click('[aria-label="Avvia condivisione"]');
  }
